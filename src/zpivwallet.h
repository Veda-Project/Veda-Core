...
#ifndef VEDA_ZVEDAWALLET_H
#define VEDA_ZVEDAWALLET_H
...
public:
    CzVEDAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZVEDA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZVEDA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //VEDA_ZVEDAWALLET_H
