...
public:
    ...
    bool WriteZVEDASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZVEDASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZVEDASeed_deprecated(uint256& seed);
    bool EraseZVEDASeed();
    bool EraseZVEDASeed_deprecated();

    bool WriteZVEDACount(const uint32_t& nCount);
    bool ReadZVEDACount(uint32_t& nCount);
