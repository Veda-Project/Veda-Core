...
CzVEDAWallet::CzVEDAWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVEDASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVEDASeed_deprecated()) {
                    LogPrintf("%s: Updated zVEDA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzVEDAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVEDACount(nCountLastUsed);
    else if (!walletdb.ReadZVEDACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVEDAWallet::Lock()
{
    seedMaster = 0;
}

void CzVEDAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVEDAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVEDA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVEDAWallet::LoadMintPoolFromDB()
{
...
void CzVEDAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVEDAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVEDAWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzVEDAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVEDA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzVEDAWallet::SeedToZVEDA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVEDAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVEDAWallet::GenerateDeterministicZVEDA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVEDAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVEDAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
