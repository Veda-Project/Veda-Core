...
CzVEDATracker::CzVEDATracker(std::string strWalletFile)
{
...
CzVEDATracker::~CzVEDATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVEDATracker::Init()
{
...
bool CzVEDATracker::Archive(CMintMeta& meta)
{
...
bool CzVEDATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVEDATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVEDATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVEDATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzVEDATracker::GetSerialHashes()
{
...
CAmount CzVEDATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVEDATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVEDATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVEDATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVEDATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVEDATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVEDATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVEDATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVEDATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVEDATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzVEDATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVEDATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVEDATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVEDATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVEDATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVEDATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVEDATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
