...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 VEDA at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zVEDA send
enum ZerocoinSpendStatus {
    ZVEDA_SPEND_OKAY = 0,                            // No error
    ZVEDA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZVEDA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZVEDA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZVEDA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZVEDA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZVEDA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZVEDA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZVEDA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZVEDA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZVEDA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZVEDA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZVEDA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZVEDA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZVEDA_SPENT_USED_ZVEDA = 14,                      // Coin has already been spend
    ZVEDA_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZVEDA_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZVedaBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZVEDAOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzVEDAWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzVEDATracker> zpivTracker;
    ...
    void setZWallet(CzVEDAWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzVEDATracker>(new CzVEDATracker(strWalletFile));
    }

    CzVEDAWallet* getZWallet() { return zwalletMain; }
    ...
    void setZVedaAutoBackups(bool fEnabled)
    {
    ...
    /** zVEDA reset */
    boost::signals2::signal<void()> NotifyzVEDAReset;
