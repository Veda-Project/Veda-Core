...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::VEDA);
    ...
    if (!settings.contains("nAnonymizeVedaAmount"))
        settings.setValue("nAnonymizeVedaAmount", 1000);

    nAnonymizeVedaAmount = settings.value("nAnonymizeVedaAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeVedaAmount"))
        SoftSetArg("-anonymizevedaamount", settings.value("nAnonymizeVedaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in veda.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVedaAmount:
            return QVariant(nAnonymizeVedaAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVedaAmount:
            nAnonymizeVedaAmount = value.toInt();
            settings.setValue("nAnonymizeVedaAmount", nAnonymizeVedaAmount);
            emit anonymizeVedaAmountChanged(nAnonymizeVedaAmount);
            break;
        case CoinControlFeatures:
